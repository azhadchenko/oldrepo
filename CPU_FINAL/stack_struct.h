...
void push(stack_t* const object, double item);
double pop(stack_t* const object);
...
stack_t stack_construct(int size_of_stack)
{
    stack_t object = {(double*)calloc(size_of_stack, sizeof(double)), 0, size_of_stack};
    return object;
};
...
double pop(stack_t* const object)
{
    ...
    double item = object->data[--object -> n];
